...
router.get('/:id', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (!hasAccess) {
            // Verificar si tiene acceso compartido (shared_access o team)
            const [shared] = await new Promise((resolve, reject) => {
                db.query(
                    `SELECT 1 FROM shared_access WHERE folder_id = ? AND shared_with_user_id = ?
                     UNION
                     SELECT 1 FROM team_members tm INNER JOIN folders f2 ON f2.team_id = tm.team_id
                     WHERE f2.folder_id = ? AND tm.user_id = ?`,
